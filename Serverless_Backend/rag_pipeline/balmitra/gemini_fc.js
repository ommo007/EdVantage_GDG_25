...
import {
  ...
  // Function to get API keys with environment fallback
  function getApiKeys(env = null) {
    return {
      GEMINI_API_KEY: getEnvVar('GEMINI_API_KEY', env),
      TAVILY_API_KEY: getEnvVar('TAVILY_API_KEY', env) || getEnvVar('tavily_api_key', env) // Support both naming conventions
    };
  }

  // Function to handle Tavily search
  async function searchTavily(query, env = null) {
  ...
  // Main function now accepts env parameter for Cloudflare Workers compatibility
  async function main(env = null) {
